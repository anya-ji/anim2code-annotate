...
export default function VideoPlayer({ url, label, videoHeight = "aspect-video" }: VideoPlayerProps) {
  ...
  const videoEl = (fullsize: boolean) => (
    <video
      ...
      className={fullsize
        ...
        : `w-full h-full object-contain transition-opacity duration-200 ${loaded ? "opacity-100" : "opacity-0"}`}
    />
  )
    ...
    <>
      <div className="flex flex-col items-center gap-1">
        ...
        <div className={`relative w-full ${videoHeight} bg-gray-100 rounded-lg overflow-hidden`}>
          ...
          {error
            ? <div className="absolute inset-0 flex items-center justify-center text-gray-400 text-xs">Video unavailable</div>
            : videoEl(false)}
          {loaded && !error && (
            <button
      ...
      {expanded && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={() => setExpanded(false)}>
          <div onClick={(e) => e.stopPropagation()}>{videoEl(true)}</div>
          <button onClick={() => setExpanded(false)} className="absolute top-4 right-4 p-2 rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors text-sm font-medium">✕</button>
        </div>
      )}
    </>
  )
